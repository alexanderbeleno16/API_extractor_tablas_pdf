...
def extraeRadicadosGenerico(path_pdf):
    ...
    for page in range(paginas):
        ...
        radicado = re.findall(r"([0-9][0-9][0-9][0-9][-][0-9][0-9][0-9][0-9][0-9][0-9]?)", texto) #<List>RADICADO 23 EJ: 2022-426253 
        ...
        myListRadicadosGenericos.append(myDiccHojaRad)
        myDiccHojaRad = {}
...
def insertar_texto_fijacion(data, json_post):
    ...
    try:
        ...
        print("Ocurrió un error al insertar el texto de fijaciones en insertar_texto_fijacion() \nError:", e)
        # exit() 
        respuestaRadicadosGenericos = extraeRadicadosGenerico(json_post['path_pdf']) 
        # print( respuestaRadicadosGenericos )   
        for diccHojaRad in respuestaRadicadosGenericos["consolidadoRadicados"] :
            print( diccHojaRad["rad"] )
...
def validarColumnasTabulaFijacion(path_archivo, json_post, opcion=1, opc_stream=False, opc_lattice=True):
    # print( "carpeta_pdf/documentos_ent_SIC/"+path_archivo )
    if opcion==2:
        columns = ["1", "2", "3", "4", "5", "6", "7", "8"]
    ...
    try:
        ...
        if len(mylistRadicado23)>0 and len(mylistEnlace)>0:
            for radicado in mylistRadicado23:
                if len(radicado)==23:
                    # EJ: 47 001 31 05 002 2020 00152 01
                    dep     = radicado[0:2]
                    mun     = radicado[2:5]
                    ent     = radicado[5:7]
                    esp     = radicado[7:9]
                    ...
                    for enlace in mylistEnlace:
                        # cadenaNumerosEnlace = enlace.replace("-","")
                        ...
                        # print( cadenaNumerosEnlace )
                                                
                        # 202100014 202114 000142021 142021 20210014 00142021
                        # El rad 23: 47 245 31 05 001 2021 00014 01 va con el enlace: https://www.ramajudicial.gov.co/documents/35605702/131566403/2021-00147+ADMISION+DE+APELACION+SENTENCIA.pdf/daebe629-a8f6-4fa7-af4e-1a39676f0554
                        
                        
                        # 202100147 2021147 001472021 1472021 20210147 01472021
                        # El rad 23: 47 288 31 05 001 2021 00147 01 va con el enlace: https://www.ramajudicial.gov.co/documents/35605702/131566403/2021-00147+ADMISION+DE+APELACION+SENTENCIA.pdf/daebe629-a8f6-4fa7-af4e-1a39676f0554
                                                
                        # if ( anio_rad in cadenaNumerosEnlace) \
                        # or (anio_rad_sin_ceros in cadenaNumerosEnlace) \
                        # or (rad_anio in cadenaNumerosEnlace) \
                        # or (rad_sin_ceros_anio in cadenaNumerosEnlace) \
                        # or (anio_rad_con_1_cero in cadenaNumerosEnlace) \
                        # or (rad_con_1_cero_anio in cadenaNumerosEnlace):
                        ...
                        # print( anio_rad , anio_rad_sin_ceros , rad_anio , rad_sin_ceros_anio , anio_rad_con_1_cero , rad_con_1_cero_anio )
                        auxx += 1
                        # print( anio +"-"+ rad )
                        # print( anio +"-"+ rad_sin_ceros )
                        # print( rad +"-"+ anio )
                        # print( rad_sin_ceros +"-"+ anio )
                        # print( anio +"-"+ rad_con_1_cero )
                        # print( rad_con_1_cero +"-"+ anio )
                        # print( enlace )
                        
                        kk1 = re.search( ""+anio +"-"+ rad+"\D", enlace)
                        limites1 = kk1.span() if kk1!=None else ""
                        
                        kk2 = re.search( ""+anio +"-"+ rad_sin_ceros+"\D", enlace)
                        limites2 = kk2.span() if kk2!=None else ""
                        
                        kk3 = re.search( ""+rad +"-"+ anio+"\D", enlace)
                        limites3 = kk3.span() if kk3!=None else ""
                        
                        kk4 = re.search( ""+rad_sin_ceros +"-"+ anio+"\D", enlace)
                        limites4 = kk4.span() if kk4!=None else ""
                        
                        kk5 = re.search( ""+anio +"-"+ rad_con_1_cero+"\D", enlace)
                        limites5 = kk5.span() if kk5!=None else ""
                        
                        kk6 = re.search( ""+rad_con_1_cero +"-"+ anio+"\D", enlace)
                        limites6 = kk6.span() if kk6!=None else ""
                        
                        # print( "limites1" , limites1 )
                        # print( "limites2" , limites2 )
                        # print( "limites3" , limites3 )
                        # print( "limites4" , limites4 )
                        # print( "limites5" , limites5 )
                        # print( "limites6" , limites6 )
                        ...
                        if(sstatus==True):
                            # print( limites1 , enlace[limites1[0] : limites1[1]]  )
                            # print( limites2 , enlace[limites2[0] : limites2[1]]  )
                            # print( limites3 , enlace[limites3[0] : limites3[1]]  )
                            # print( limites4 , enlace[limites4[0] : limites4[1]]  )
                            # print( limites5 , enlace[limites5[0] : limites5[1]]  )
                            # print( limites6 , enlace[limites6[0] : limites6[1]]  )

                            # print( auxx , len(radicado) , "El rad: " +radicado+ " va con el enlace: "+enlace )
            ...
            # print( len(mylistDicc) )
            iii=0
            # for rad in mylistRadicado23:
                
            for vv in mylistDicc:
                iii+=1
                
                # if (rad==vv["radicado"]):
                print(iii , vv["radicado"] )
...
def cruzarRadicadosConLupaCpj(mylistDiccionarioRadEnlaces, id_despacho, usuario):
    ...
    # print( len(mylistDiccionarioRadEnlaces) )
    # print( mylistDiccionarioRadEnlaces )
                ...
                if respuestaTieneAuto["rows"]>0:
                    ...
                    for auto in respuestaTieneAuto["data"]:
                        if auto['imagenes']=="HRJ.jpg" and auto['grabacion_mixta']=="1": # Si el auto es grabado por el robot, actualizo las imagenes
                            ...
                            cnt_autos_insertados += respuesta_wget['cnt_autos_insertados']
                            cnt_autos_faltantes  += respuesta_wget['cnt_autos_faltantes']
...
def consultarRadEnLupa(dicc):
    ...
    # ¿¿ SE AÑADE EL AÑO Y EL RAD EN EL WHERE ??
    query += "AND c.cpj_ano         = '"+dicc['anio']+"' "
    query += "AND c.cpj_radicacion  = '"+dicc['rad']+"' "
